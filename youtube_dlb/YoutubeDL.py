...
class YoutubeDL(QThread):
    ...
    def __init__(self, params=None, auto_init=True,user_opt={}):
    ...
    def to_screen(self, message, skip_eol=False):
        ...
        my_message={"ID":self.user_opt["ID"],"message":message}
        self.mySignal.emit(my_message)
        ...
        for ie in ies:
            ...
            try:
                ...
                if ie_result is None:  # Finished already (backwards compatibility; listformats and friends should be moved here)
                    break
                if isinstance(ie_result, list):  #测试是否是List实例
    ...
    def process_video_result(self, info_dict, download=True):
        ...
        thumbnail = info_dict.get('thumbnail')
        if thumbnail:
            info_dict['thumbnail'] = sanitize_url(thumbnail) #处理一个URL
        elif thumbnails:
    ...
    def process_info(self, info_dict):
        ...
        if len(info_dict['title']) > 200:        # 电影的title
        ...
        if self.params.get('simulate', False):
            #self.params={'nocheckcertificate': False, 'proxy': 'https://127.0.0.1:1080'}
            return
    ...
    def run(self):
        url_list=self.user_opt.get('url_list')
        retcode=None
        try:
            retcode=self.download(url_list)
            print('retcode:', retcode)
        except Exception as er:
            print('Exception:',er)
            print('retcode:', retcode)
            message={"ID":self.user_opt["ID"],"Code":101}
            self.exceptSignal.emit(message)
            print("downloading exception!")
